...
export const writeUniverseToFile = async (universe: Universe) => {
    ...
    const universeData = {
      ...
      planets: universe.planets.map(planet => ({
        coordinates: planet.coordinates.format(), // Use the format method of Coordinates class
      ...
      monsters: universe.monsters.map(monster => ({
        coordinates: monster.coordinates.format() // Use the format method of Coordinates class
      }))
    };
